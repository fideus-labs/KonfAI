...
TOOL_DESCRIPTIONS: dict[str, str] = {
    "browse_dataset": (
        ...
        "Outputs: tree, candidate_dataset_roots, common_groups, incomplete_groups, and next_actions. "
    ...
    "design_config_strategy": (
        ...
        "A group's role is is_input in the model graph: input = fed to the network, target = held out of "
        "the input as supervision. It cannot be read off the group name (CT is the input for segmentation "
        "and the target for MR->CT synthesis), so inspect_dataset leaves it null and it is settled here, "
        "with the user. "
    ...
    "describe_app": (
        ...
        "Next: run_app_infer / list_app_parameters / import_app when it fits (next_actions reflect "
    ...
    "import_app": (
        ...
        "is used exactly as published; import_app is the tier for everything else: editing the config, fine-tuning "
        "(run_resume with weights_only=True), or wiring the app into a larger experiment. "
        "It copies the app's config(s), custom code, and .pt checkpoints into the session root "
    ...
    "package_app_from_session": (
        ...
        "KonfAI app bundle, so a from-scratch run can also finish as a "
    ...
    "run_resume": (
        ...
        "from scratch (import_app + run_resume(weights_only=True) is THE fine-tune path for published apps). "
    ...
    "plan_transform": (
        ...
        "Outputs: {ok, config_path, world_size, report, verdict_counts, budget, budget_bytes, needs_attention[], "
        "over_budget[]}. A non-empty over_budget means run_transform would refuse before writing anything; "
        "needs_attention lists at most 50 entries, so trust verdict_counts for the totals. "
...
SOLVE_TASK_PROMPT = (
    ...
    "2. FINE-TUNE FROM AN APP. If no app is usable as-is but one is a close starting point, "
    "import_app it into the session and train from its weights with run_resume(weights_only=True) "
    "on the user's dataset; package_app_from_session can then turn the result into a bundle.\n"
