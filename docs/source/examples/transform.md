...
This is the shortest way to see KonfAI do something real. There is no model to
train, no checkpoint to fetch and no dataset to download: `make_dataset.py`
writes 3.5 MB of synthetic volumes, and both configs run on CPU in about a
minute. `Transform_demo.ipynb` runs the whole thing cell by cell, in Colab too.
...
- {doc}`../config_guide/transform`: every key, every refusal, and the twelve
