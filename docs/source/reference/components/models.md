...
node types. Models whose graph contains a leaf the registry does not know (diffusion samplers, StyleGAN, ConvNeXt, VoxelMorph's warping components) are
...
Beyond the Python classes above, KonfAI ships a **declarative model catalog**: addressable directly as `classpath: default|<Name>.yml`, discoverable by name,
