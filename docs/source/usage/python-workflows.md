...
dry-run twin), `konfai.evaluate`, `konfai.predict` and `konfai.train`. One engine, two spellings —
everything below builds the same config tree the YAML file would hold and hands it to the same
...
Every workflow entry point accepts the config **tree as a dict** wherever it accepts a file path —
that alone is the sweep idiom for TRAIN: the resolved config each run keeps *is* the record of what
