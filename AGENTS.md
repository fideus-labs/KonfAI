...
| `TRANSFORM` | `Transform.yml` | `Transformer:` | **No model.** Read a dataset, run a chain per case, `Write` the result. Plan first; an existing output is skipped, so a run resumes |

Each run writes a **workspace** keyed by `train_name` (`name` for TRANSFORM): `Checkpoints/`, `Statistics/` (TensorBoard + the resolved-config snapshot), `Predictions/`, `Evaluations/` (metric JSON), `Transforms/` (per-rank logs, the config copy, `plan.txt` and `outputs.json`, but **not the transformed data**, which lands wherever each `Write:` points, in the user's own tree). `Dataset/` is the *input* data directory, not a run output.
