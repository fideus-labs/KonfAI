...
classpaths** (`ResidualEncoderUNet.yml`, `Model:RegistrationNet`), never KonfAI's internal module paths, that is why the `models`→`models.python` move did not break bundles. A bundle resolves from a **Local dir**,
