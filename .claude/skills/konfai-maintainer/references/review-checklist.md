...
The **watchlist** entries are traps confirmed by the 2026-07-19 deep audit; all have since been fixed and
pinned by tests: the job now is to not reintroduce them.
...
| `data/patching/`, `data/data_manager/` | **Patch order out (disassemble) == order in (Accumulator)**; all patches of a case stay on **one DDP rank** for predict/eval | Reassemble an identity through the pipe and assert byte-identical, incl. `overlap>0` and non-divisible shapes. |
| `data/transform/` | `transform_shape()` is **exact**; `patch_locality` never over-claims (a wrong locality silently corrupts the streamed path); the declaration must match what `__call__` does | `test_transform_locality_contract.py` enumerates every built-in and proves streamed == whole-volume cut: a new streamable stage is covered the day it lands; check your case table entry if defaults are not a meaningful streaming case. |
| `data/reduction.py` | `voxel_local`/`incremental`/`working_multiple` are load-bearing: a wrong `voxel_local=True` **corrupts the streamed output** (the gate checks nothing else); a wrong `working_multiple` is a plan the run exceeds | Fold a cohort streamed vs whole and compare; check the plan's peak against a measured RSS for the operator. |
| `konfai/api.py` | A caller's config FILE is copied: the write-back never lands on it; `KONFAI_*` env restored around every call; one workflow per process | `tests/unit/test_api.py`; assert the caller's file bytes are untouched after a run. |
| `transformer.py` | Plan verdicts (`STREAM`/`LOAD`/`WHOLE-VOLUME`/`SKIP`/`REDUCE`/`REFUSED`) are the run's actual routing; `--plan` probes destinations with a real region-write (not read-only) | `test_transformer_workflow.py` pins verdicts and plan lines; `--plan` short-circuits in `main.py` (the distributed wrapper would drop the kwarg). |
| `utils/dataset/` backends | A `SUPPORTED_BACKEND_FORMATS` token is **not an extension** (`:itktransform` writes `<group>.h5`); only extensions are probed on disk; `storage_form` matches from the right, longest first: a dot in the stem belongs to the name | `test_itk_transform_backend.py`, `test_imaging_formats.py`; adding a backend token must not enter the on-disk probe set. |
...
| `network/network/` | `state_dict` does **not** recurse into nested Networks; `outputs_criterions` keys equal a module's **dotted path** (`:`/`.` load-bearing); alias lists are **positional** | Build + save + load + RESUME a *nested* model; assert optimizer/scheduler/`_it` continue. |
| `metric/measure/` | A `Criterion.forward` returns a `Tensor` (loss) or `(value, dict)` (metric); metric state must reset per split | Instantiate and run the criterion on a tiny CPU tensor; a metric must not accumulate across train+val. |
| `predictor/` | Streamed output is **byte-identical** to whole-volume; the mode is stated when a case is not window-bounded | A/B with `KONFAI_STREAMED_WRITES=0`; check the writer fingerprint (streamed sink omits `CenterOfRotation`). |
...
- **Config union coercion**: `overlap: 0.25` once bound `0` (no blend). Fixed; pinned by
  `test_config.py::test_apply_config_union_keeps_the_value_type_over_lossy_coercion`. Any new
  union-typed config key still needs a test against a non-first member.
...
- **Geometry axis order**: `Resample` / `Elastix` mixing `(z,y,x)` grids with `(x,y,z)` spacing and
