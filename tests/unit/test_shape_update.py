...
"""ShapeUpdate: the shape residual of a displacement field, streamed."""
...
from konfai.data.transform import ShapeUpdate, Write
...
def test_shape_update_streams_and_equals_the_whole_volume(tmp_path: Path) -> None:
    source = _field(tmp_path)
    manager = _manager(source, [ShapeUpdate(step=0.25), Write(f"{tmp_path / 'out'}:h5")])
...
def test_a_streamed_shape_update_never_reads_the_volume(tmp_path: Path, monkeypatch) -> None:
    source = _field(tmp_path)
    manager = _manager(source, [ShapeUpdate(step=0.25), Write(f"{tmp_path / 'out'}:h5")])

    def refuse(*args, **kwargs):
        raise AssertionError("the shape update assembled the field")
