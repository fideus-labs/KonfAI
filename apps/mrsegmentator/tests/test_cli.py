...
"""Smoke tests for the thin mrsegmentator_konfai CLI wrapper: the argparse tree built by
...
from mrsegmentator_konfai import cli


def _run(monkeypatch: pytest.MonkeyPatch, argv: list[str]) -> int:
    monkeypatch.setattr(sys, "argv", ["mrsegmentator-konfai", *argv])
...
def test_repo_is_pinned() -> None:
    assert cli.MR_SEGMENTATOR_KONFAI_REPO == "VBoussot/MRSegmentator-KonfAI"
