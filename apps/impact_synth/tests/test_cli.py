...
"""Smoke tests for the thin impact_synth_konfai CLI wrapper: the argparse tree built by
...
from impact_synth_konfai import cli


def _run(monkeypatch: pytest.MonkeyPatch, argv: list[str]) -> int:
    monkeypatch.setattr(sys, "argv", ["impact-synth", *argv])
...
def test_repo_is_pinned() -> None:
    assert cli.IMPACT_SYNTH_KONFAI_REPO == "VBoussot/ImpactSynth"
    assert callable(cli.main)


@pytest.mark.parametrize("subcommand", ["synthesize", "eval", "pipeline"])
def test_expected_subcommands_are_wired(monkeypatch: pytest.MonkeyPatch, subcommand: str) -> None:
    # --help on a valid subcommand exits 0 (the parser accepts the app's own vocabulary).
