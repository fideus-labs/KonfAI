...
"""Smoke tests for the thin impact_seg_konfai CLI wrapper: the argparse tree built by
...
from impact_seg_konfai import cli


def _run(monkeypatch: pytest.MonkeyPatch, argv: list[str]) -> int:
    monkeypatch.setattr(sys, "argv", ["impact-seg", *argv])
...
def test_repo_is_pinned() -> None:
    assert cli.IMPACT_SEG_KONFAI_REPO == "VBoussot/ImpactSeg"
