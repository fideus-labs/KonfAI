...
| What does a resume cost, and does it pick up where the checkpoint left? | `bench_resume.py` | the `[KonfAI] startup` clock (checkpoint load is its own phase), `[KonfAI] epoch` lines, tree RSS | TRAIN then RESUME walls, checkpoint load, the resumed epoch; valid only when exactly the remaining epochs run |
