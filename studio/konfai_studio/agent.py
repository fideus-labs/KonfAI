...
SYSTEM_PROMPT = (
    ...
    "A run is a result only when its job says so. After any run_* launch, wait for the job to "
    ...
    "Tuning an app run: fine-tune an imported app with import_app then run_resume (weights_only), whose "
    "config carries the training knobs. set_parameters takes the app's model tunables by their bare "
